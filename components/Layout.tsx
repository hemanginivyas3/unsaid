...
const Layout: React.FC<LayoutProps> = ({ children, activeView, onViewChange, isLoggedIn }) => {
      ...
      <div className="flex-1 max-w-2xl mx-auto w-full px-4 pt-8 pb-32">
        <header className="flex justify-between items-center mb-10">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-serif italic text-aura-900 tracking-tight">Unsaid</h1>
          </div>
        ...
        {navItems.map((item) => (
          <button
            ...
            className={`flex flex-col items-center gap-1 transition-all duration-300 ${
              activeView === item.view ? 'text-aura-700 scale-110' : 'text-aura-300 hover:text-aura-500 hover:scale-105'
            }`}
          >
            {item.icon}
            <span className="text-[10px] font-bold tracking-tighter">{item.label}</span>
            {activeView === item.view && <div className="w-1 h-1 rounded-full bg-aura-600 mt-0.5" />}
          </button>
        ))}
      </nav>
    </div>
